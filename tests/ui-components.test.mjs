...
test("emits NagarAI light, dark and reduced-motion styles", async () => {
  const css = await readCssTree(path.join(root, "dist"));

  assert.match(css, /--green:\s*#087a5b/);
  assert.match(css, /--red:\s*#f32948/);
  assert.match(css, /\.site-header/);
  assert.match(css, /nagarai-civic-operations\.png/);
  assert.match(css, /\.page-intro aside\{[^}]*background:(?:transparent|0 0)/);
  assert.match(css, /\.stats-grid \.stat-card\{[^}]*background:linear-gradient/);
  assert.match(css, /\.site-footer\{[^}]*min-height:96px/);
...
test("emits chart themes for NagarAI media dark mode", async () => {
